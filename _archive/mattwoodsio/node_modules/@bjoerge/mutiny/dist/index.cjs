...
var parse = require('./_chunks/parse-8ISYfcYA.cjs');
...
function patch(id, patches, options) {
  return {
    type: "patch",
    id,
    patches: isObject.arrify(patches),
    ...(options ? {
      options
    } : {})
  };
...
function patchToSanity(patch) {
  ...
  if (op.type === "unset") {
    return {
      unset: [stringify.stringify(path)]
    };
  }
  if (op.type === "insert") {
    return {
  ...
  if (op.type === "diffMatchPatch") {
    return {
      diffMatchPatch: {
        [stringify.stringify(path)]: op.value
      }
    };
  }
  if (op.type === "inc") {
    return {
      inc: {
        [stringify.stringify(path)]: op.amount
      }
    };
  }
  if (op.type === "dec") {
  ...
  if (op.type === "truncate") {
    ...
    return {
      unset: ["".concat(stringify.stringify(path), "[").concat(range, "]")]
    };
  }
  if (op.type === "upsert") {
    return {
      unset: op.items.map(item => stringify.stringify([...path, {
        _key: item._key
      }])),
      insert: {
        [op.position]: stringify.stringify([...path, op.referenceItem]),
  ...
  if (op.type === "assign") {
    return {
      set: Object.fromEntries(Object.keys(op.value).map(key => [stringify.stringify(path.concat(key)), op.value[key]]))
    };
  }
  if (op.type === "unassign") {
    return {
...
function getInsertPatches(patch) {
  ...
  const path = parse.parse(patch.insert[position]);
...
function encodePatchMutation(id, patch) {
  ...
  const path = stringify.stringify(patch.path);
  if (op.type === "unset") {
...
function decodePatchMutation(mutation) {
  ...
  const path = parse.parse(serializedPath);
...
exports.CompactEncoder = index;
exports.CompactFormatter = compact;
exports.SanityEncoder = index$1;
