"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
Object.defineProperty(exports, "__esModule", { value: true });
var rxjs_1 = require("rxjs");
var useObservable_1 = require("../useObservable");
var react_1 = require("react");
var react_2 = require("@testing-library/react");
var useAsObservable_1 = require("../useAsObservable");
var wait = function (ms) { return new Promise(function (resolve) { return setTimeout(resolve, ms); }); };
// NOTE: Jest runs NODE_ENV=test by default, which enables development flags for React
test('Strict mode should trigger double mount effects and re-renders', function () { return __awaiter(void 0, void 0, void 0, function () {
    function ObservableComponent() {
        (0, react_1.useEffect)(function () {
            mountCount++;
        }, []);
        var observedValue = (0, useObservable_1.useObservable)(observable);
        returnedValues.push(observedValue);
        return (0, react_1.createElement)(react_1.Fragment, null, observedValue);
    }
    var subject, observable, returnedValues, mountCount, rerender;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                subject = new rxjs_1.BehaviorSubject(0);
                observable = subject.asObservable();
                returnedValues = [];
                mountCount = 0;
                rerender = (0, react_2.render)((0, react_1.createElement)(react_1.StrictMode, null, (0, react_1.createElement)(ObservableComponent))).rerender;
                expect(mountCount).toEqual(2);
                expect(returnedValues).toEqual([0, 0]);
                return [4 /*yield*/, wait(10)];
            case 1:
                _a.sent();
                (0, react_2.act)(function () { return subject.next(1); });
                expect(returnedValues).toEqual([0, 0, 1, 1]);
                (0, react_2.act)(function () { return subject.next(2); });
                expect(returnedValues).toEqual([0, 0, 1, 1, 2, 2]);
                expect(mountCount).toEqual(2);
                return [2 /*return*/];
        }
    });
}); });
test('Strict mode should unsubscribe the source observable on unmount', function () {
    var subscribed = false;
    var observable = new rxjs_1.Observable(function () {
        subscribed = true;
        return function () {
            subscribed = false;
        };
    });
    function ObservableComponent() {
        (0, useObservable_1.useObservable)(observable);
        return (0, react_1.createElement)(react_1.Fragment, null);
    }
    var rerender = (0, react_2.render)((0, react_1.createElement)(react_1.StrictMode, null, (0, react_1.createElement)(ObservableComponent))).rerender;
    expect(subscribed).toBe(true);
    rerender((0, react_1.createElement)(react_1.StrictMode, null, (0, react_1.createElement)('div')));
    expect(subscribed).toBe(false);
});
test('useAsObservable should work in strict mode', function () { return __awaiter(void 0, void 0, void 0, function () {
    function ObservableComponent(props) {
        var count$ = (0, useAsObservable_1.useAsObservable)(props.count);
        var count = (0, useObservable_1.useObservable)(count$);
        returnedValues.push(count);
        return (0, react_1.createElement)(react_1.Fragment, null, 'ok');
    }
    var returnedValues, rerender;
    return __generator(this, function (_a) {
        returnedValues = [];
        rerender = (0, react_2.render)((0, react_1.createElement)(react_1.StrictMode, null, (0, react_1.createElement)(ObservableComponent, { count: 0 }))).rerender;
        expect(returnedValues).toEqual([0, 0]);
        rerender((0, react_1.createElement)(react_1.StrictMode, null, (0, react_1.createElement)(ObservableComponent, { count: 1 })));
        expect(returnedValues).toEqual([0, 0, 0, 0, 1, 1]);
        return [2 /*return*/];
    });
}); });
